...
int main()
{
    ...
    vector<int> cards_count(winning_numbers_vec.size(), 1);
    ...
    for (int i = 0; i < winning_numbers_vec.size(); i++)
    {
        ...
        for (int h = i + 1; (h < i + hits_count + 1) && (h < winning_numbers_vec.size()); h++)
        {
            cards_count[h] += cards_count[i];
        }
        hits_count = 0;
    }
    for (int& n : cards_count)
    {
        total += n;
    }
